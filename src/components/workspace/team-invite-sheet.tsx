...
export function TeamInviteSheet({
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      const { row, acceptUrl, emailSent, verificationRequested, verificationTarget } =
        await createTeamInvitationViaProxy({
        name: name.trim(),
        email: email.trim(),
        role,
      ...
      if (emailSent) {
      ...
      } else if (verificationRequested && verificationTarget) {
        toast.success(
          `Invitación creada. AWS envió un correo de verificación a ${verificationTarget}. ` +
            "Confirmalo y volvé a invitar, o compartí el enlace manualmente.",
          acceptUrl
            ? {
                action: {
                  label: "Copiar enlace",
        ...
        toast.success("Invitación creada. Copiá el enlace y compartilo con la persona invitada.", {
          action: {
            label: "Copiar enlace",
            onClick: () => {
              void navigator.clipboard.writeText(acceptUrl);
